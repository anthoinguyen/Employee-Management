...
function removeLinkImgOnArr(arr, name) {
...
function findIdFromName(arr, name) {
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].name == name) {
      return arr[i].id;
    }
...
function findIdFromVali(arr, vali) {
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].vali == vali) {
      return arr[i].id;
    }
...
function removeItemByVali(arr, vali) {
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].vali == vali) {
      arr.splice(i, 1);
      break;
...
function makeInputDanger(a){
  $(a).css('border','2px solid red');
}
function makeInputDefault(a){
  $(a).css('border','');
}
function clearInputField(a){
  $(a).find('input, textarea, select').val("");
}
